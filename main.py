...
async def search_in_channels(query, client):
    ...
    async for dialog in client.iter_dialogs():
        if dialog.is_channel:
            channel_count += 1
            logger.info(f"Searching in channel: {dialog.title} (ID: {dialog.entity.id})")
            try:
                async for message in client.iter_messages(dialog.id, search=query, limit=10):  # کاهش limit به 10
                    if message.text:
